<template>
  <el-dialog :model-value="show" title="添加用户" @close="close">
    <el-form ref="form" class="max-width-400" :model="item" :rules="rules" label-width="100px" label-suffix=":">
      <el-form-item v-if="isCreate" label="用户账号" prop="username">
        <el-input v-model="item.username" />
      </el-form-item>
      <el-form-item v-if="isCreate" label="用户密码" prop="password">
        <el-input v-model="item.password" type="password" show-password />
      </el-form-item>
      <el-form-item label="用户姓名" prop="name">
        <el-input v-model="item.name" />
      </el-form-item>
      <el-form-item v-if="item.id != user.id" label="账号状态" prop="state">
