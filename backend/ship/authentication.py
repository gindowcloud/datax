...
from container.user import tasks
...
def authenticate(db: Session, username: str, password: str):
    user = tasks.get_user_by_username(db, username)
    if not user:
        return False
    ...
    access_token = tasks.put_access_token(db, user.id, token)
...
def session(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    access_token = tasks.get_access_token(db, token)
    if access_token is None:
        raise credentials_exception
    else:
        user = tasks.get_user_by_id(db, access_token.user_id)
        if user is None:
            raise credentials_exception
    return user
